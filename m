...
-- Per-station maximum possible payout exposure.
-- This is NOT max stake.
-- This means no station can place bets that could pay more than this total
-- on any single roulette result.
local MAX_STATION_POSSIBLE_PAYOUT = 75000

local LAST_ROLLS_FILE = "/roulette_last_rolls.json"
local MAX_LAST_ROLLS = 4

-- Dark green custom button background for UNDO/SPIN
local DARK_BUTTON_BG = colors.brown
local DARK_BUTTON_BG_HEX = 0x178530
local DARK_BUTTON_BG_BLIT = "c"

-- =========================================================
-- === Better RNG Helper
-- =========================================================

local rngSpinCounter = 0

local function reseedRouletteRNG()
    rngSpinCounter = rngSpinCounter + 1

    local epoch = 0
    if os.epoch then
        epoch = os.epoch("utc") or 0
    end

    local clockPart = math.floor((os.clock() or 0) * 1000000)
    local timePart = math.floor((os.time() or 0) * 100000)
    local idPart = os.getComputerID() or 0
    ...
    print("Roulette RNG reseeded:", seed)
end

local function rouletteRandom(minVal, maxVal)
    for i = 1, 5 do
        math.random()
    end
...
for _, station in ipairs(STATIONS) do
    ...
    if station.monitor.setPaletteColor then
        ...
        station.monitor.setPaletteColor(DARK_BUTTON_BG, DARK_BUTTON_BG_HEX)
    end
end
...
local monitorHitboxes = {}

local roundState = "open"
...
local function getStationPayoutOnResult(stationIndex, result)
    local payout = 0
...
local function getMaxAllowedByPayoutCap(stationIndex, zone)
    local maxAllowed = math.huge

    for _, result in ipairs(ALL_ROULETTE_RESULTS) do
        local won, mult = betWon(zone, result)

        if won then
            local currentPayout = getStationPayoutOnResult(stationIndex, result)
            local remainingPayoutRoom = MAX_STATION_POSSIBLE_PAYOUT - currentPayout
            local allowedHere = math.floor(remainingPayoutRoom / mult)

            if allowedHere < maxAllowed then
                maxAllowed = allowedHere
            end
        end
    end
...
local function debugRecordResult(result)
    debugCounts[result] = (debugCounts[result] or 0) + 1

    print("Result:", result, "Count:", debugCounts[result])

    local total = 0
    for _, c in pairs(debugCounts) do
        total = total + c
    end

    if total % 20 == 0 then
        print("---- Result counts after " .. total .. " spins ----")
        for _, num in ipairs(WHEEL_ORDER_BY_LAMP_ASC) do
            print(num .. ": " .. tostring(debugCounts[num] or 0))
        end
    end
end
...
local function performFinalSpinSlowdown()
    ...
    reseedRouletteRNG()
    local resultNumberIndex = rouletteRandom(1, #RESULT_LAMPS)
    ...
    debugRecordResult(resultNumber)
...
-- =========================================================
-- === Monitor Drawing: 114x114 2x3 Subpixel Frame
-- =========================================================
...
local function loadSubpixelFrame(path)
    ...
    if not file then
        error("Could not open " .. path .. ". Save your 114x114 art as frame.txt on this computer.")
    end
    ...
    if not width or not height then
        error("First line of " .. path .. " must be like: 114 114")
    end

    if width % 2 ~= 0 or height % 3 ~= 0 then
        error(path .. " must be sized for 2x3 packing. Got " .. width .. "x" .. height)
    end
...
local backgroundFrame = loadSubpixelFrame(FRAME_FILE)

local function drawPackedFrame(mon, frame)
...
local function drawBackground(mon)
    if mon.setPaletteColor then
        pcall(function()
            mon.setPaletteColor(DARK_BUTTON_BG, DARK_BUTTON_BG_HEX)
        end)
    end

    mon.setBackgroundColor(colors.green)
    mon.clear()
    drawPackedFrame(mon, backgroundFrame)
end

-- =========================================================
-- === Touch Hitboxes for frame.txt
-- =========================================================

local zoneRects = {}

local function addRectZone(name, x1, y1, x2, y2)
    table.insert(zoneRects, {
        name = name,
        x1 = x1,
        y1 = y1,
        x2 = x2,
...
-- zero boxes
addRectZone("0", 2, 7, 5, 12)
addRectZone("00", 2, 13, 5, 18)

-- main number grid
local topRow = {3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36}
local midRow = {2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35}
local botRow = {1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34}

local function addNumberRow(nums, y1, y2)
    for i, n in ipairs(nums) do
        local x1 = 5 + ((i - 1) * 4)

        -- Give the right-edge separator/boundary column to the number on the LEFT.
        -- Example: x=9,y=7 should hit 3, not 6.
        -- This intentionally overlaps adjacent number hitboxes by 1 x-column.
        -- Since zones are checked in insertion order left-to-right, the left number wins.
        addRectZone(tostring(n), x1, y1, x1 + 4, y2)
    end
end

addNumberRow(topRow, 7, 12)
addNumberRow(midRow, 13, 17)
addNumberRow(botRow, 18, 23)

-- right column bets
addRectZone("2to1_top", 53, 7, 57, 12)
addRectZone("2to1_mid", 53, 13, 57, 17)
addRectZone("2to1_bot", 53, 18, 57, 23)

-- dozens
addRectZone("1st12", 5, 24, 21, 27)
addRectZone("2nd12", 22, 24, 37, 27)
addRectZone("3rd12", 38, 24, 53, 27)

-- outside bets
addRectZone("1to18", 2, 28, 11, 31)
addRectZone("even", 12, 28, 21, 31)
addRectZone("red", 22, 28, 29, 31)
addRectZone("black", 30, 28, 37, 31)
addRectZone("odd", 38, 28, 45, 31)
addRectZone("19to36", 46, 28, 57, 31)

-- chips
addRectZone("chip_100", 2, 32, 7, 33)
addRectZone("chip_500", 8, 32, 14, 33)
addRectZone("chip_1000", 16, 32, 23, 33)
addRectZone("chip_2500", 25, 32, 32, 33)

-- controls
addRectZone("clear", 35, 32, 44, 33)
addRectZone("undo", 48, 32, 55, 33)
addRectZone("spin", 32, 34, 54, 37)

local function getZoneAt(x, y)
    for _, zone in ipairs(zoneRects) do
        if x >= zone.x1
            and x <= zone.x2
            and y >= zone.y1
            and y <= zone.y2 then
...
-- Static labels/numbers are part of frame.txt now.
local function drawBoardLabels(mon)
end

-- =========================================================
-- === Countdown / Dynamic Overlay
-- =========================================================
...
local function writeClipped(mon, x, y, text, textColor, bgColor, maxWidth)
    ...
    local available = math.min(maxWidth, monW - x + 1)
    if available < 1 then
        return
    end
    ...
    -- Important: do NOT pad with spaces.
    -- The frame is redrawn first every display refresh, so writing only the
    -- actual string avoids black/green bars cutting into the SPIN button/art.
    mon.setCursorPos(x, y)
    mon.setBackgroundColor(bgColor or colors.green)
...
local function drawStation(station)
    ...
    -- Keep dynamic status tucked into the lower-left so it does not fight the table hitboxes.
    writeClipped(mon, 2, 34, getCountdownText(station), colors.orange, colors.green, 56)

    if displayPlayer then
        local bal = balances[displayPlayer] or station.balance or 0
        writeClipped(mon, 2, 35, "Bal: $" .. tostring(bal), colors.yellow, colors.green, 56)

        local chipText = station.selectedChip and ("Chip $" .. station.selectedChip) or "Select chip"
        local bettingText = bettingOpen() and "Bets open" or "Bets closed"

        writeClipped(mon, 2, 36, tostring(displayPlayer), colors.white, colors.green, 56)
        writeClipped(mon, 2, 37, chipText .. "    " .. bettingText, colors.white, colors.green, 56)
    else
        writeClipped(mon, 2, 35, "Stand at table", colors.yellow, colors.green, 56)
        writeClipped(mon, 2, 36, "No player", colors.white, colors.green, 56)
        writeClipped(mon, 2, 37, "Select chip    " .. (bettingOpen() and "Bets open" or "Bets closed"), colors.white, colors.green, 56)
    end

    writeClipped(mon, 2, 38, station.message or "", colors.yellow, colors.green, 56)
...
local function placeBet(station, zone)
    ...
    local stationIndex = station.index

    local zoneMax = getZoneMax(zone)
    local stationZoneCurrent = getStationZoneBet(stationIndex, zone)
    local zoneAvailable = math.max(0, zoneMax - stationZoneCurrent)

    local payoutCapAvailable = getMaxAllowedByPayoutCap(stationIndex, zone)

    local allowable = math.min(chip, zoneAvailable, payoutCapAvailable)

    if allowable <= 0 then
        if zoneAvailable <= 0 then
            station.message = "Your zone max reached."
        elseif payoutCapAvailable <= 0 then
            station.message = "Max payout reached."
        else
    ...
    betHistory[player] = betHistory[player] or {}
    table.insert(betHistory[player], {
        zone = zone,
        amount = allowable
    })
...
local function undoLastBet(station)
    if not station.activePlayer then return end
    ...
    local player = station.activePlayer
    local history = betHistory[player]

    if not history or #history <= 0 then
        station.message = "No bet to undo."
        return
    end

    local lastBet = table.remove(history)
    local zone = lastBet.zone
    local amount = lastBet.amount or 0

    if amount <= 0 or not zone then
        station.message = "Undo failed."
        return
    end

    if placedBets[player] and placedBets[player][zone] then
        placedBets[player][zone] = placedBets[player][zone] - amount

        if placedBets[player][zone] <= 0 then
            placedBets[player][zone] = nil
        end
    ...
    local newBal = sendBalanceChange(
        player,
        amount,
        "ROULETTE UNDO BET: refunded $" .. amount .. " from " .. zone
    )

    balances[player] = newBal
    station.balance = newBal

    logRoulette(player, "UNDO BET: refunded $" .. amount .. " from " .. zone)

    station.message = "Undid $" .. amount .. " on " .. zone
end
...
local function clearPlayerBets(station)
    ...
    local player = station.activePlayer
    local refund = refundPlayerBets(player, "ROULETTE REFUND: cleared bets")
    betHistory[player] = nil
...
if os.epoch then
    startupSeed = startupSeed + (os.epoch("utc") or 0)
end

startupSeed = startupSeed
    + math.floor((os.clock() or 0) * 1000000)
    + math.floor((os.time() or 0) * 100000)
    + ((os.getComputerID() or 0) * 7919)

startupSeed = startupSeed % 2147483647

math.randomseed(startupSeed)

for i = 1, 50 do
    math.random()
end

print("Startup RNG seed:", startupSeed)
...
print("Per-station max possible payout:", MAX_STATION_POSSIBLE_PAYOUT)
print("Subpixel frame UI enabled: frame.txt")
print("Frame hitboxes embedded. Boundary fix + chip/status row enabled.")
print("Dark button background enabled: #178530")
